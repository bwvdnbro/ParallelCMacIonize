...
if args.messagefile:
    ...
    for iproc in range(nproc):
        mdata[mdata[:, 0] == iproc, 5] = (
            mdata[mdata[:, 0] == iproc, 5] - tmin[iproc]
        ) * tconv[iproc] * tconv_in_s[iproc] + tmin_in_s[iproc]
    ...
    for iproc in range(nproc):
        for jproc in list(range(iproc)) + list(range(iproc + 1, nproc)):
            # ...and tags
            for itag in range(ntag):
                # send events
                sends = mdata[
                    (mdata[:, 2] == 0)
                    & (mdata[:, 0] == iproc)  # send events
                    & (mdata[:, 3] == jproc)  # from rank iproc
                    & (mdata[:, 4] == itag)  # to rank jproc
                ]  # with tag itag
                recvs = mdata[
                    (mdata[:, 2] == 1)
                    & (mdata[:, 3] == iproc)  # receive events
                    & (mdata[:, 0] == jproc)  # from rank iproc
                    & (mdata[:, 4] == itag)  # on rank jproc
                ]  # with tag itag

                if not len(sends) == len(recvs):
                    print(
                        "Sizes do not match (sends: {0}, receives: {1})!".format(
                            len(sends), len(recvs)
                        )
                    )
                    exit()
