...
if len(ptime.shape) > 1:
    ...
    tconv = [1.0 / (line[2] - line[1]) for line in ptime]
    total_time = np.array([line[3] for line in ptime]).mean()
else:
    tmin = [ptime[1]]
    tconv = [1.0 / (ptime[2] - ptime[1])]
    total_time = ptime[3]
...
def plot_file(name):
    ...
    for iproc in range(nproc):
        ...
        for i in range(nthread):
            ...
            bar = [
                (
                    (task[1] - tmin[iproc]) * tconv[iproc] * total_time,
