...
argparser.add_argument(
    "-r", "--reemission_probability", action="store", default=0.0
)
...
ncell = int(
    np.round(
        (os.path.getsize("neutral_fractions.dat") / (10 * 8)) ** (1.0 / 3.0)
    )
)
...
Rs = (0.75 * Q / (1.0 - PR) / (np.pi * nH ** 2 * alphaH)) ** (1.0 / 3.0)
...
data = np.memmap(
    "neutral_fractions.dat", dtype=np.float64, shape=(ncell ** 3, 10), mode="r"
)
# compute the radii
pos = data[:, 0:3]
...
Ibin, _, _ = stats.binned_statistic(
    r, data[:, 3], statistic="mean", bins=rbin_edges
)
I2bin, _, _ = stats.binned_statistic(
    r, data[:, 3] ** 2, statistic="mean", bins=rbin_edges
)
