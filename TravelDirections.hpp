...
inline static int output_to_input_direction(const int output_direction) {
  ...
  default:
    ...
    cmac_error("Unknown output direction: %i", output_direction);
    return -1;
  }
}
...
inline bool is_compatible_output_direction(const double *direction,
  ...
  default:
    // something went wrong
    cmac_error("Invalid output direction: %i", output_direction);
    return false;
  }
}
...
inline bool is_compatible_input_direction(const double *direction,
  ...
  default:
    // something went wrong
    cmac_error("Invalid input direction: %i", input_direction);
    return false;
  }
}
 ...
 */
inline int get_output_direction(const int mask) {
  switch (mask) {
  case 0:
